...
void DealData::InsertRcvData(std::string msgId,std::string dataStr,ByteSeq data,int id)
{
	LOG log("L3MsgSnd");
	try
	{
		string sql = "INSERT INTO UACS_L3MSG_RECEIVE (ID,MSGID,DATA,DATASTR,REC_TIME) VALUES (:p_ID,:p_MSGID,:p_DATA,:p_DATASTR,:p_REC_TIME)";
...
int DealData::GetMaxIDRcv()
{
	LOG log("L3MsgSnd");
	int ID = 0;
	try
	{
		string sql = "SELECT MAX(ID) AS ID FROM UACS_L3MSG_RECEIVE";
		iDA::Command cmd(&CDataAccess::GetInstance()->m_DBCon, sql);    // command object
		cmd.Execute();
		if (cmd.FetchNext())
		{
