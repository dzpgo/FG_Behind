...
//-----KIBH01-----20221218
bool SQL4L3:: DelMatOutInfo(string planNO)
{

	bool ret=false;
	string functionName="SQL4L3 :: DelMatOutInfo()";
	LOG log(functionName, AUTO_CATCH_PID);
	try
	{
		string strSql="";
		strSql="DELETE FROM UACS_L3_MAT_OUT_INFO WHERE PLAN_NO = :p_PLAN_NO ";
		...
		log.Debug()<<__FUNCTION__<<" DelMatOutInfo Sucess! "<<endl;
...
//-----KIBH01-----20221218
bool SQL4L3:: InsertMatOutInfo(string operateFlag, string planNO, string bofNO, int count, string matCode_1, int weight_1, string matCode_2, int weight_2, string matCode_3, int weight_3, string matCode_4, int weight_4, string matCode_5, int weight_5, string matCode_6, int weight_6, string matCode_7, int weight_7, string matCode_8, int weight_8, string matCode_9, int weight_9, string matCode_10, int weight_10)
{

	bool ret=false;
	string functionName="SQL4L3 :: InsertMatOutInfo()";
	LOG log(functionName, AUTO_CATCH_PID);
	try
	{
		string strSql="";
		strSql="INSERT INTO  UACS_L3_MAT_OUT_INFO( ";
		strSql += "OPER_FLAG, ";
		strSql += "PLAN_NO, ";
		strSql += "BOF_NO, ";
		strSql += "CYCLE_COUNT, ";
		...
		strSql += " VALUES( :p_OPER_FLAG,";
		strSql += " :p_PLAN_NO, ";
		strSql += " :p_BOF_NO, ";
		strSql += " :p_CYCLE_COUNT, ";
		strSql += " :p_MAT_CODE_1, ";
		strSql += " :p_WEIGHT_1, ";
		strSql += " :p_MAT_CODE_2, ";
		strSql += " :p_WEIGHT_2, ";
		...
		strSql += " :p_MAT_CODE_9, ";
		strSql += " :p_WEIGHT_9, ";
		strSql += " :p_MAT_CODE_10, ";
		strSql += " :p_WEIGHT_10, ";
		strSql += " :p_REC_TIME )";

		iDA::Command cmd(&CDataAccess::GetInstance()->m_DBCon, strSql);    // command object

		cmd.Param("p_OPER_FLAG") = operateFlag;
		cmd.Param("p_PLAN_NO") =planNO;
		cmd.Param("p_BOF_NO") =bofNO;
		cmd.Param("p_CYCLE_COUNT") =(long)count;
		cmd.Param("p_MAT_CODE_1") =matCode_1;
		cmd.Param("p_WEIGHT_1") =(long)weight_1;
		cmd.Param("p_MAT_CODE_2") =matCode_2;
		cmd.Param("p_WEIGHT_2") =(long)weight_2;
		cmd.Param("p_MAT_CODE_3") =matCode_3;
		cmd.Param("p_WEIGHT_3") =(long)weight_3;
		cmd.Param("p_MAT_CODE_4") =matCode_4;
		cmd.Param("p_WEIGHT_4") =(long)weight_4;
		cmd.Param("p_MAT_CODE_5") =matCode_5;
		cmd.Param("p_WEIGHT_5") =(long)weight_5;
		cmd.Param("p_MAT_CODE_6") =matCode_6;
		cmd.Param("p_WEIGHT_6") =(long)weight_6;
		cmd.Param("p_MAT_CODE_7") =matCode_7;
		cmd.Param("p_WEIGHT_7") =(long)weight_7;
		cmd.Param("p_MAT_CODE_8") =matCode_8;
		cmd.Param("p_WEIGHT_8") =(long)weight_8;
		cmd.Param("p_MAT_CODE_9") =matCode_9;
		cmd.Param("p_WEIGHT_9") =(long)weight_9;
		cmd.Param("p_MAT_CODE_10") =matCode_10;
		cmd.Param("p_WEIGHT_10") =(long)weight_10;
		cmd.Param("p_REC_TIME") =iDA::DateTime::Now();
		cmd.Execute();
		...
		log.Debug()<<__FUNCTION__<<" InsertMatOutInfo Sucess! "<<endl;
...
//-----KIBH02-----20221218
bool SQL4L3:: UpdateMatCarRelationInfo(string arriveFlag, string hopperNO, string positionNO, string loadFlag)
{

	bool ret=false;
	string functionName="SQL4L3 :: UpdateMatCarRelationInfo()";
	LOG log(functionName, AUTO_CATCH_PID);
	try
	{
		string strSql="";
		strSql=" UPDATE  UACS_L3_MAT_CRA_RELATION  SET  ";
		strSql += "ARRIVE_FLAG = :p_ARRIVE_FLAG, ";
		strSql += "HOPPER_NO = :p_HOPPER_NO, ";
		strSql += "LOAD_FLAG = :p_LOAD_FLAG, ";
		strSql += "REC_TIME = :p_REC_TIME ";
		strSql += "WHERE   POSITION_NO = :p_POSITION_NO ";



		iDA::Command cmd(&CDataAccess::GetInstance()->m_DBCon, strSql);    // command object

		cmd.Param("p_ARRIVE_FLAG") = arriveFlag;
		cmd.Param("p_HOPPER_NO") =hopperNO;
		cmd.Param("p_LOAD_FLAG") =loadFlag;
		cmd.Param("p_REC_TIME") =iDA::DateTime::Now();
		cmd.Param("p_POSITION_NO") =positionNO;
		cmd.Execute();
		...
		log.Debug()<<__FUNCTION__<<" UpdateMatCarRelationInfo Sucess! "<<endl;
	}
	catch(iDA::Exception &x)
	{
		try
...
bool SQL4L3:: UpdYardMapGridInfo(string gridNO, string matCode, string compCode, int curWt)
{
	...
	try
	{
		...
		cmd.Param("p_MAT_CODE") = matCode;
		cmd.Param("p_COMP_CODE") = compCode;
		cmd.Param("p_MAT_WGT") = (long)curWt;
...
//-----KIBH04-----称量实绩
bool SQL4L3:: UpdateMatOutInfo_WT(string planNO, string hopperNO, int netWeight)
{
	bool ret=false;
	string functionName="SQL4L3 :: UpdateMatOutInfo_WT()";
	LOG log(functionName, AUTO_CATCH_PID);
	try
	{		
		string strSql="";
		strSql="UPDATE  UACS_L3_MAT_OUT_INFO  SET  ";
		strSql += "CAR_NO = :p_CAR_NO, ";
		strSql += "MAT_NET_WT = :p_MAT_NET_WT, ";
		strSql += "WT_TIME = :p_WT_TIME ";
		strSql += "WHERE   PLAN_NO = :p_PLAN_NO ";

		iDA::Command cmd(&CDataAccess::GetInstance()->m_DBCon, strSql);    // command object

		cmd.Param("p_CAR_NO") =hopperNO;
		cmd.Param("p_MAT_NET_WT") =(long)netWeight;
		cmd.Param("p_PLAN_NO") =planNO;
		cmd.Param("p_WT_TIME") =iDA::DateTime::Now();
		cmd.Execute();
...
bool SQL4L3:: MatWeightInfoExist(string workSeqNO)
{
	...
	try
	{
		long nCount = 0;
		string strSql="";
		strSql = "SELECT * FROM UACS_L3_MAT_WEIGHT_INFO  WHERE WEIGH_NO =:p_WEIGH_NO ";

		iDA::Command cmd(&CDataAccess::GetInstance()->m_DBCon, strSql);    // command object

		cmd.Param("p_WEIGH_NO") = workSeqNO;
		cmd.Execute();
		while (cmd.FetchNext())
		{
			nCount = 1;
...
bool SQL4L3:: InsertMatWeightInfo(SmartData sd)
{
	...
	try
	{
		...
		cmd.Param("p_OPERATE_FLAG") = operateFlag;
		cmd.Param("p_WORK_SEQ_NO") ="1";
		//cmd.Param("p_HC_WORK_SEQ_NO") =hcWorkSeqNO;
		cmd.Param("p_TRUCK_NO") =carNO;
		//cmd.Param("p_WEIGH_APP_NO") =weighAppNo;
		...
		cmd.Param("p_MAT_PROD_CODE") =matcode;
		cmd.Param("p_MAT_WT") =(long)matweigh;
...
bool SQL4L3:: InsertMatWeightRGVInfo(SmartData sd)
{
	...
	try
	{
		...
		{			
			//库位/料格号
			stockPlaceCode = sd["KIBH05_DETAIL"][i]["STOCK_PLACE_CODE"].toString();	
			//物料代码
			matCode = sd["KIBH05_DETAIL"][i]["MAT_CODE"].toString();
			//钢制代码
			compCode = sd["KIBH05_DETAIL"][i]["COMP_CODE"].toString();
			//分摊重量
			wgt = sd["KIBH05_DETAIL"][i]["WGT"].toInt();
...
bool SQL4L3:: UpdateMatWeightInfo(SmartData sd)
{
	...
	try
	{
		...
		strSql += "TRUCK_NO = :p_TRUCK_NO, ";
		//strSql += "WEIGH_APP_NO = :p_WEIGH_APP_NO, ";
		//strSql += "WEIGH_APP_TIME = :p_WEIGH_APP_TIME, ";
		//strSql += "REC_CREATOR = :p_REC_CREATOR, ";
		strSql += "WEIGH_NO = :p_WEIGH_NO, ";
		strSql += "MAT_PROD_CODE = :p_MAT_PROD_CODE, ";
		strSql += "MAT_WT = :p_MAT_WT, ";
		...
		strSql += "UPD_TIME = :p_UPD_TIME ";
		strSql += "WHERE  WEIGH_NO = :p_WEIGH_NO ";
